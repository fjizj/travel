...
{	
	...
	$.fn.myInit = function(){
		...
		prePage = $("#footer").attr("pre");
		if(prePage){
			var n = prePage.indexOf("#");
			if(n >= 0){
				var key = prePage.substr(n,prePage.length);
				prePage = prePage.substr(0,n)+".html"+key;
			} else {
				prePage = prePage+".html";
			}
			$("li.previous")
				.removeClass("disabled")
				.children("a").attr("href", "day_"+prePage);
		}
		nexPage = $("#footer").attr("next");
		if(nexPage){
			$("li.next")
				.removeClass("disabled")
				.children("a").attr("href", "day_"+nexPage+".html");
		}
	}
})();
